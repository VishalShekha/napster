...
interface Song {
  ...
  uploadDate: string; // ISO date or date string
  cover?: string;
}
...
const router = useRouter();
  async function fetchSongs() {
    ...
    setError(null);
    try {
      // NOTE: according to your backend index.js the route for user songs is mounted at "/user-songs"
  ...
  useEffect(() => {
    fetchSongs();
    // If you want polling or re-fetch on focus, add logic here
  }, []);
  ...
  const topPlays = songs.length > 0 ? Math.max(...songs.map((s) => s.plays || 0)) : 0;
    ...
    <div className="flex h-screen bg-background">
      ...
      <main className="flex-1 overflow-auto pb-24">
        <div className="p-8">
          ...
          {/* Loading / Error */}
          {loading && <p className="text-muted-foreground mb-4">Loading songs...</p>}
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
              <Card className="bg-gradient-to-br from-primary/20 to-chart-2/20 border-primary/20">
                <CardContent className="p-6">
                  <div className="text-2xl font-bold text-primary mb-1">{publishedCount}</div>
              ...
              <Card className="bg-gradient-to-br from-chart-3/20 to-chart-4/20 border-chart-3/20">
                <CardContent className="p-6">
                  <div className="text-2xl font-bold text-chart-3 mb-1">{draftCount}</div>
                  <div className="text-sm text-muted-foreground">Drafts</div>
                </CardContent>
              </Card>
              <Card className="bg-gradient-to-br from-chart-5/20 to-primary/20 border-chart-5/20">
                <CardContent className="p-6">
                  <div className="text-2xl font-bold text-chart-5 mb-1">{totalPlays.toLocaleString()}</div>
                  <div className="text-sm text-muted-foreground">Total Plays</div>
                </CardContent>
              </Card>
              <Card className="bg-gradient-to-br from-chart-2/20 to-chart-3/20 border-chart-2/20">
                <CardContent className="p-6">
                  <div className="text-2xl font-bold text-chart-2 mb-1">{topPlays.toLocaleString()}</div>
          ...
          <Card>
            <CardContent className="p-0">
              <div className="space-y-0">
                {/* If still loading show simple placeholders or the original loading text */}
                {loading && (
                  <div className="p-6">
                    <p className="text-muted-foreground">Loading songs list...</p>
                  </div>
                )}

                {!loading && !error && songs.length === 0 && (
                  <div className="p-6">
                    <p className="text-muted-foreground">You have no songs yet.</p>
                  </div>
                )}
                ...
                {!loading && !error &&
                  songs.map((song) => (
                    <div
                      key={song.id} // use the unique id from the API (not index)
